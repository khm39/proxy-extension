...
interface AppState {
  ...
  lastError: ProxyError | null;    // 最後のプロキシエラー
...
│       ├── import-profiles.ts
│       ├── toggle-dev-mode.ts
│       ├── get-connection-logs.ts
│       ├── clear-connection-logs.ts
│       └── clear-error.ts
├── lib/
│   ├── types.ts              # 型定義・定数
│   ├── proxy-manager.ts      # プロキシ設定・認証ハンドラーロジック
│   ├── storage.ts            # ストレージ操作
│   └── profile-utils.ts      # プロファイル操作のユーティリティ
...
| `import-profiles.ts` | `{ profiles }` | `{ success, imported?, skipped?, error? }` |
| `toggle-dev-mode.ts` | `{ enabled? }` | `{ devMode }` |
| `get-connection-logs.ts` | `{}` | `{ logs: ConnectionLogEntry[] }` |
| `clear-connection-logs.ts` | `{}` | `{ success }` |
| `clear-error.ts` | `{}` | `{ success }` |
